...
# #Start with data munged using the ldsc package
# trait1File="/Volumes/BIOBANK/ldsc/munged_summary_stat/FI3-word_interpolation.sumstats.gz"
# 
# trait2File="/Volumes/BIOBANK/ldsc/munged_summary_stat/FI8-chained_arithmetic.sumstats.gz"
# 
# #Load trait 1 data and calculate Zs
# d1 = na.omit(read.table(gzfile(trait1File),header=TRUE,sep="\t",stringsAsFactors = FALSE))
# 
...
# 
# #Load LD scores
# setwd('/Volumes/BIOBANK/ldsc/ldscore/pre-computed_LDsc/eur_w_ld_chr/')
# LDfiles <- list.files(path =".", pattern = "ldscore.gz")
# temp <- lapply(LDfiles, fread, sep="\t")
# LDscores_unsorted <- rbindlist( temp )
# d3= LDscores_unsorted[with(LDscores_unsorted, order(CHR, BP)), ]
# 
# #ldscoresFile=""/Volumes/BIOBANK/ldsc/ldscore/pre-computed_LDsc/eur_w_ld_chr""
...
# 
# #Run LCV-need to setwd to directory containing LCV package
# setwd("/Volumes/BIOBANK/LCV/R")
# source("/Volumes/BIOBANK/LCV/R/RunLCV.R")
# 
# LCV = RunLCV(data$L2,data$Z.x,data$Z.y)
# sprintf("Estimated posterior gcp=%.3f(%.3f), gcp p-value=%.3f; estimated rho=%.3f(%.3f)",LCV$gcp.pm, LCV$gcp.pse, LCV$pval.gcpzero.2tailed, LCV$rho.est, LCV$rho.err)
# 
# LCV
# 
# 
